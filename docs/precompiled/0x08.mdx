...
In the group $G\_2$, points on the extension field $F\_\{p^2\}$ are represented with coordinates (x, y) where `x = x_c0 + x_c1 * i` and `y = y_c0 + y_c1 * i`, with each component (x_c1, x_c0, y_c1, y_c0) encoded as 32 bytes.
