...
Input must be exactly 128 bytes. The field element is represented as an element of Fp2 = Fp[u]/(u² + 1), encoded as c0 + c1*u. Both components must be valid elements of the BLS12-381 base field.
